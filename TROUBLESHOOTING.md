...
这个状态通常不是 volume 本身损坏，而是宿主机上还有一个挂载点指向该 volume 的 `_data` 目录。默认 bridge 目录是 `/mnt/revopia`，备份残留路径通常是 `/mnt/revopia/<friendly-name>`，恢复残留路径通常是 `/mnt/revopia/restore/<friendly-name>`。
...
```bash
docker ps -a --filter label=revopia=true
```
...
如果标签里有 `revopia.mode=restore` 或 `revopia.restore-target=true`，再看 `revopia.name`。这个值就是恢复路径里的 friendly name，默认残留挂载点应该是 `/mnt/revopia/restore/<friendly-name>`。

```bash
docker volume inspect <volume-name> --format '{{ index .Labels "revopia.name" }}'
findmnt /mnt/revopia/restore/<friendly-name>
```

如果你通过 `--bridge-source` 或 `REVOPIA_BRIDGE_SOURCE` 改过 bridge 目录，请把 `/mnt/revopia` 替换成实际宿主机路径。如果输出里的源路径指向 `/var/lib/docker/volumes/<volume-name>/_data` 或对应 Docker 数据目录，就说明这个恢复挂载还残留在 bridge 下。

例如目标 volume 是 `revopia_test-db-data20260508-153000`，inspect 后看到 `revopia.name=db-primary`，那么要检查的路径就是下面这个。

```bash
findmnt /mnt/revopia/restore/db-primary
```

如果 `findmnt` 显示它的源路径指向 `/var/lib/docker/volumes/revopia_test-db-data20260508-153000/_data`，就可以确认 Docker volume 删除失败是这个残留传播挂载导致的。
...
```bash
docker volume inspect <volume-name> --format '{{ .Mountpoint }}'
findmnt -R /mnt/revopia
```

备份残留通常在 `/mnt/revopia/<friendly-name>`，恢复残留通常在 `/mnt/revopia/restore/<friendly-name>`。
...
```bash
sudo umount /mnt/revopia/<friendly-name>
```

恢复路径的残留挂载使用下面的命令。

```bash
sudo umount /mnt/revopia/restore/<friendly-name>
```
...
```bash
sudo fuser -vm /mnt/revopia/<friendly-name>
```
...
```bash
sudo lsof +f -- /mnt/revopia/<friendly-name>
```
...
```bash
findmnt /mnt/revopia/<friendly-name>
...
```bash
findmnt /mnt/revopia/restore/db-primary
docker volume rm revopia_test-db-data20260508-153000
```
...
```bash
sudo umount -l /mnt/revopia/<friendly-name>
```

恢复路径使用下面的命令。

```bash
sudo umount -l /mnt/revopia/restore/<friendly-name>
```
